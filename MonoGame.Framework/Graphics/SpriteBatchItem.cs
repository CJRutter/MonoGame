...
namespace Microsoft.Xna.Framework.Graphics
{
    internal class SpriteBatchItem : IComparable<SpriteBatchItem>
    {
        ...
        public void Set(Vector2 topLeft, Vector2 topRight, Vector2 bottomLeft, Vector2 bottomRight, Color color, Vector2 texCoordTL, Vector2 texCoordBR, float depth)
        {
            vertexTL.Position.X = topLeft.X;
            vertexTL.Position.Y = topLeft.Y;
            vertexTL.Position.Z = 0f;
            vertexTL.Color = color;
            vertexTL.TextureCoordinate.X = texCoordTL.X;
            vertexTL.TextureCoordinate.Y = texCoordTL.Y;

            vertexTR.Position.X = topRight.X;
            vertexTR.Position.Y = topRight.Y;
            vertexTR.Position.Z = 0f;
            vertexTR.Color = color;
            vertexTR.TextureCoordinate.X = texCoordBR.X;
            vertexTR.TextureCoordinate.Y = texCoordTL.Y;

            vertexBL.Position.X = bottomLeft.X;
            vertexBL.Position.Y = bottomLeft.Y;
            vertexBL.Position.Z = 0f;
            vertexBL.Color = color;
            vertexBL.TextureCoordinate.X = texCoordTL.X;
            vertexBL.TextureCoordinate.Y = texCoordBR.Y;

            vertexBR.Position.X = bottomRight.X;
            vertexBR.Position.Y = bottomRight.Y;
            vertexBR.Position.Z = 0f;
            vertexBR.Color = color;
            vertexBR.TextureCoordinate.X = texCoordBR.X;
            vertexBR.TextureCoordinate.Y = texCoordBR.Y;
